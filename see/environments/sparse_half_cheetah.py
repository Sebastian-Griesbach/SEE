...
XML_FILE = (
    "half_cheetah.xml"  # sparse_half_cheetah.xml for rendering the reward boarder
)
...
class SparseHalfCheetahEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release.
    ...
    def __init__(
        self,
        xml_file: str = XML_FILE,  # sparse_half_cheetah.xml for rendering the reward boarder
        frame_skip: int = 5,
        ...
        **kwargs,
        ...
        # passing render_mode="human" to MujocoEnv.__init__ will cause an error, therefore we filter None values
    ...
    def step(self, action):
        ...
        reward, reward_info = self._get_rew(x_position_after, action)
    ...
    def _get_rew(self, x_position: float, action):
        sparse_reward = 1.0 if x_position > self._goal_distance else 0.0
...
class LongSparseHalfCheetahEnv(SparseHalfCheetahEnv):
    def __init__(
        self,
        xml_file=XML_FILE,
        frame_skip=5,
...
class AdverseSparseHalfCheetahEnv(SparseHalfCheetahEnv):
    def __init__(
        self,
        xml_file=XML_FILE,
        frame_skip=5,
        default_camera_config=DEFAULT_CAMERA_CONFIG,
        goal_distance=5,
        ctrl_cost_weight=0.1,  # controll cost of original environment is 0.1
        reset_noise_scale=0.1,
        exclude_current_positions_from_observation=False,
        **kwargs,
    ):
