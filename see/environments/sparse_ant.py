...
class SparseAntEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release
    ...
    def __init__(
        ...
        ctrl_cost_weight: float = 0.0,  # original value was 0.5
        contact_cost_weight: float = 0.0,  # original value was 5e-4
        healthy_reward: float = 0.0,  # original value was 1.0
    ...
    def step(self, action):
        ...
        reward, reward_info = self._get_rew(self.data.qpos[0], action)
    ...
    def _get_rew(self, x_position: float, action):
        sparse_reward = 0 if x_position < self._goal_distance else 1
        control_reward = -self._ctrl_cost_weight * self.control_cost(action)
        contact_reward = -self._contact_cost_weight * self.contact_cost

        reward = sparse_reward + self.healthy_reward + control_reward + contact_reward

        reward_info = {
            "sparse_reward": sparse_reward,
            "reward_ctrl": control_reward,
...
class AdverseSparseAntEnv(SparseAntEnv):
    def __init__(
        self,
        xml_file="ant.xml",
        frame_skip=5,
        default_camera_config=DEFAULT_CAMERA_CONFIG,
        goal_distance=4,
        ctrl_cost_weight: float = 0.5,  # original value
        contact_cost_weight: float = 5e-4,  # original value
        healthy_reward: float = 0.0,  # original value is 1.0
        main_body=1,
