...
class SparseHopperEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release.
    ...
    def __init__(
        ...
        ctrl_cost_weight: float = 0.0,  # original value is 1e-3
    ...
    def _get_rew(self, x_position: float, action):
        ...
        ctrl_reward = -self._ctrl_cost_weight * self.control_cost(action)

        reward = sparse_reward + healthy_reward + ctrl_reward
...
class AdverseSparseHopperEnv(SparseHopperEnv):
    def __init__(
        self,
        xml_file="hopper.xml",
        frame_skip=4,
        default_camera_config=DEFAULT_CAMERA_CONFIG,
        goal_distance=1,
        ctrl_cost_weight: float = 1e-3,  # original value is 1e-3
        healthy_reward: float = 0.0,  # original value is 1.0
